...
ALTER TABLE proxy ADD COLUMN out_ip TEXT;
...
insert into proxy_temp (id, proxy, port, timezone, country, ISP, alive, status)
SELECT id, proxy, port, timezone, country, ISP, alive, status
FROM proxy;
...
CREATE INDEX IF NOT EXISTS idx_status ON proxy_temp(status);
